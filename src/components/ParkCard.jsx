...
export default function ParkCard({ park, size = 'medium' }) {
  const imageHeight = size === 'large' ? 'h-72 md:h-96' : size === 'medium' ? 'h-52 md:h-64' : 'h-44';

  return (
    <Link to={`/park/${park.id}`} className="group block no-underline">
      <div className="bg-white rounded-xl overflow-hidden border border-[#e5e3da] shadow-sm hover:shadow-md transition-all duration-300 group-hover:-translate-y-1">
        ...
        <div className="p-4">
          <div className="flex items-start justify-between gap-2 mb-1.5">
            <h3 className="text-lg font-semibold text-[#2b2823] group-hover:text-[#3d5a3e] transition-colors leading-tight" style={{ fontFamily: "'Source Serif 4', Georgia, serif" }}>
              {park.name}
            </h3>
